...
void RegisterRequest(int32_t p1, int32_t p2) {
    if (allRequests.find(p1) == allRequests.end() && allRequests.find(p2) == allRequests.end()) {
    ...
    if (HasRequested(requester)) {
        ...
        if (!isBattleActive(requester) && !isBattleActive(requested)) {
            try {
                ...
                std::cout << e.what();
                bot.getApi().sendMessage(message->chat->id, fmt::format("player {} not registered", e.what()));
                return;
            }
        } else {
...
void AskBattleCommand(TgBot::Bot &bot, TgBot::Message::Ptr message) {
    ...
    if (args.size() > 0) {
        ...
        if (requestedID != INVALID_ID) {
            ...
            bot.getApi().sendMessage(message->chat->id, args.at(1) + " Type yes to accept");
        } else {
