...
void registerBattleCommand(TgBot::Bot &bot) {
  bot.getEvents().onCommand("battle", [&bot](TgBot::Message::Ptr message) {
    auto player1 =
        FetchPlayer(boost::lexical_cast<std::string>(message->from->id));
    ...
    if (!isBattleActive(player1->Uid) && !isBattleActive(player2->Uid)) {
      ...
      std::cout << battle->Players[player1->Uid]->Team[0]->Nickname
                << std::endl;

      bot.getApi().sendMessage(message->chat->id,
                               battle->generateBattleSummary());
      bot.getApi().sendMessage(message->chat->id,
                               "player 1" +
                                   battle->generateMoveSummary(player1->Uid));
      bot.getApi().sendMessage(message->chat->id,
                               "player 2" +
                                   battle->generateMoveSummary(player2->Uid));
