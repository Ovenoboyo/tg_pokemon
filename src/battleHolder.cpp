...
int BattleHolder::calculateDamage(Player attacker, Player defender) {
  ...
  int defenceStat = getStats(
      defender.Team[0]->baseStats.Attack, defender.Team[0]->IVStats.Attack,
      defender.Team[0]->EVStats.Attack, Attackerlevel, false);

  float attackModifier = getAttackModifier(
      defender.Team[0]->type, this->playedMove[attacker.Uid].GetType());
...
void BattleHolder::HandlePlayerChoice(UID uid, int moveNo) {
  this->playedMove.insert(
      std::pair<UID, Move>(uid, getMoveFromIndex(uid, moveNo)));
